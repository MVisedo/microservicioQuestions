...
export class RabbitDirectConsumer {
    ...
    constructor(private queue: string, private exchange: string, routingKey?: string) {
        this.routingKey = routingKey !== undefined ? routingKey : queue;
    }
    ...
    async init() {
        try {
            ...
            channel.bindQueue(queue.queue, exchange.exchange, this.routingKey);

            channel.consume(queue.queue,
                (message) => {
                    const rabbitMessage: IRabbitMessage = JSON.parse(message.content.toString());
                    if (!rabbitMessage.type && this.processors.size === 1) {
                        const type = this.processors.keys().next().value;
                        // The message from catalog is not a IRabbitMessage, so we need to wrap it.
                        const wrappedMessage: IRabbitMessage = {
                            type: type,
                            message: rabbitMessage
                        };
                        this.processors.get(type)(wrappedMessage);
                    } else if (this.processors.has(rabbitMessage.type)) {
